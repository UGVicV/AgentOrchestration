...
# Characters forbidden in handler names.
...
class AgentRegistry:
    ...
    def register(
        ...
        try:
            safe_name = _validate_name(
                "name", name
            )
            safe_type = _validate_name(
            ...
            group = safe_type.split(".")[0]
            if group not in self._index:
                self._index[group] = []
            self._index[group].append(
                agent_id
            )
            logger.info(
                "Registered agent %s"
    ...
    def delete(
        ...
        if (
            group in self._index
            and agent_id
            in self._index[group]
        ):
            self._index[group].remove(
                agent_id
            )
        return True
